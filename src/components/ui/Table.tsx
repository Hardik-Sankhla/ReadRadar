...
export function ResourceTable({ data, isLoading = false }: Props) {
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    ...
    return <span className="inline-block ml-1 text-primary">{sortOrder === 'asc' ? '↑' : '↓'}</span>;
  };
      ...
      <div className="mt-2 font-mono text-xs">
        {Object.entries(dna).map(([topic, score]) => (
          <div key={topic} className="flex items-center mb-1">
            <span className="w-24 text-gray-400">{topic}</span>
              ...
              return (
                <React.Fragment key={entity.id}>
                  <tr className="cursor-pointer group hover:bg-[#262626]" onClick={() => toggleRow(entity.id)}>
                    ...
                    <td className="font-bold text-white transition-colors relative whitespace-normal min-w-[200px]">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isFoundational && <span className="px-1.5 py-0.5 bg-danger/20 text-danger text-[10px] border border-danger/50 uppercase tracking-wider font-bold">Classic</span>}
                        {!isFoundational && isNew && <span className="px-1.5 py-0.5 bg-success/20 text-success text-[10px] border border-success/50 uppercase tracking-wider font-bold">New</span>}
                        {!isFoundational && !isNew && isTrending && <span className="px-1.5 py-0.5 bg-warning/20 text-warning text-[10px] border border-warning/50 uppercase tracking-wider font-bold">Trending</span>}
                      </div>
                    </td>
                  ...
                  <tr className="bg-[#1a1a1a]">
                      <td colSpan={7} className="p-6 border-b border-border expanded-panel">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 font-body text-sm overflow-hidden">
                          ...
                          <div className="col-span-1 md:col-span-4 flex flex-col gap-6">
                            <div>
                              <h4 className="font-bold mb-2 uppercase tracking-wide font-heading text-xs text-primary">Why Read This</h4>
                            ...
                            {entity.learning_outcomes && entity.learning_outcomes.length > 0 && (
                              <div>
                                <h4 className="font-bold mb-2 uppercase tracking-wide font-heading text-xs text-primary">Learning Outcomes</h4>
                            ...
                            <div className="flex gap-4">
                              <div className="neo-card p-3 flex-1 bg-muted border-border">
                                <div className="text-[10px] text-gray-400 uppercase font-mono mb-1">Difficulty</div>
                                <div className="font-bold text-white text-sm">{entity.difficulty || "Unknown"}</div>
                              </div>
                              <div className="neo-card p-3 flex-1 bg-muted border-border">
                                <div className="text-[10px] text-gray-400 uppercase font-mono mb-1">Time Req</div>
                          ...
                          <div className="col-span-1 md:col-span-4 flex flex-col gap-6">
                            <div>
                              <h4 className="font-bold mb-2 uppercase tracking-wide font-heading text-xs text-gray-300">Knowledge DNA</h4>
                              {renderKnowledgeDNA(entity.knowledge_dna)}
                            </div>

                            {entity.prerequisites && entity.prerequisites.length > 0 && (
                              <div>
                                <h4 className="font-bold mb-2 uppercase tracking-wide font-heading text-xs text-gray-300">Prerequisites</h4>
                            ...
                            {entity.skip_if && entity.skip_if.length > 0 && (
                              <div className="bg-danger/10 border border-danger/30 p-3 rounded-sm">
                                <h4 className="font-bold mb-1 uppercase tracking-wide font-heading text-[10px] text-danger">Skip This If...</h4>
                          ...
                          <div className="col-span-1 md:col-span-4">
                            <div className="neo-card p-4 border-border h-full flex flex-col justify-between">
                              <dl className="space-y-4">
                                <div>
                                  <dt className="text-gray-400 font-heading text-[10px] uppercase mb-1 tracking-wider">Verification Status</dt>
                                  <dd className="space-y-2">
                                    <div className="flex justify-between items-center text-xs font-mono">
                                      <span className="text-gray-300">Official Source</span>
                                      <span className={entity.verification?.official ? "text-success font-bold" : "text-gray-400"}>
                                    ...
                                    <div className="flex justify-between items-center text-xs font-mono">
                                      ...
                                      <span className="text-warning font-bold">★ {entity.verification?.community_score || "N/A"}</span>
                                    </div>
                                  </dd>
                                </div>
                                ...
                                <div>
                                  <dt className="text-gray-400 font-heading text-[10px] uppercase mb-1 tracking-wider">Metadata</dt>
        ...
        <div className="flex justify-between items-center font-mono text-sm">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="px-4 py-2 border-2 border-border bg-muted hover:bg-border disabled:opacity-50 transition-colors"
            >
              Prev
            </button>
